...
ggsave(sprintf("output/%s_%s.png",condition_type,met),height = 10.5, width = 6)
ggsave(sprintf("output/%s_%s.pdf",condition_type,met),height = 10.5, width = 6)
}
...
```{r}
color_data <- read.csv("input/strain_colors.csv")
...
pivoted_color %>%
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) 
ggsave(sprintf("output/strain_colors.png"),height = 10.5, width = 5)
ggsave(sprintf("output/strain_colors.pdf"),height = 10.5, width = 5)
```

# Figure S1A: Delta pH 
```{r}
pH_data <- read.csv("input/220812_pH_table_lagtime_clean_strains_no_outliers_with_pH.csv")
...
ggsave("output/Figure_S1A_pH.pdf",width=12,height=8)
```
# Figure S1B: OD vs. categories
```{r}
pH_data %>% ggplot(aes(x = corrected_individual_rate, y = corrected_individual_OD,color=SILVA_family)) + 
  ...
  theme_bw() +
  theme(strip.background = element_blank(),legend.position = "bottom")
ggsave("output/Figure_S1B_osmolality_vs_pH.pdf",width=12,height=8)
```
